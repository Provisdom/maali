...
(defn handle-state-command
  ...
  (match command
         [:init init-session] (-> init-session
                                  (rules/insert ::specs/Visibility {::specs/visibility :all})
                                  (rules/fire-rules))
         [:insert :todo todo] (rules/insert session ::specs/Todo todo)
         [:insert-many :todos todos] (apply rules/insert session ::specs/Todo todos)
         ...
         [:update :todo id attrs] (rules/upsert session ::specs/Todo (rules/query-fn ::todo/todo-by-id :?todo :?id id)
                                                merge attrs)
         [:update :visibility visibility] (rules/upsert session ::specs/Visibility
                                                        (rules/query-fn ::todo/visibility :?visibility)
                                                        assoc ::specs/visibility visibility)
         [:complete-all :todos done] (rules/upsert session ::specs/Todo
...
(defn query-result->command
  ...
  (match binding-map-entry
         [::todo/visible-todos todos] [:render :todo-list (->> todos (map :?todo) (sort-by ::specs/id))]
         [::todo/visibility visibility] [:render :visibility (-> visibility first :?visibility ::specs/visibility)]
